...
import {Form, Select, Input, message, Button, DatePicker, TimePicker, Checkbox, Radio, Transfer} from 'antd'
import './editcalendar.less'
import PageHead from '../../components/pageheader'
import moment from "moment";
import {connect} from "react-redux";
import WrapedCheckBox from "../../components/wrapedcheckbox";
import {calendaradd, getuserlist} from '../../api'
    ...
    handleSubmit = event => {
        ...
        this.props.form.validateFields(async (err, values) => {
            if (!err) {
                ...
                try {
                    const result = await calendaradd(this.props.user.id, name, startdate.format('YYYY-MM-DD'), starttime.format('HH:mm'), enddate.format('YYYY-MM-DD'), endtime.format('HH:mm'), title, titlecolor, content, remarks, cc, ismsg)
                    if (result.ret==200) {
